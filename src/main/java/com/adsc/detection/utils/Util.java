...
import com.adsc.detection.Debuger;
...
public class Util {
    ...
    public static void drawRectOnMat(Rect r, Mat finalImage, Scalar scalar) {
        Scalar color = scalar;
        Point A = new Point(r.x, r.y), B = new Point(r.x + r.width, r.y),
                C = new Point(r.x + r.width, r.y+ r.height), D = new Point(r.x, r.y + r.height);
        Imgproc.line(finalImage, A, B, color, 4, 4, 0);
        Imgproc.line(finalImage, B, C, color, 4, 4, 0);
        Imgproc.line(finalImage, C, D, color, 4, 4, 0);
        Imgproc.line(finalImage, D, A, color, 4, 4, 0);
    }
     ...
     */
    public static void drawQonMat(double [][] Q, Mat finalImage, Scalar scalar) {
        Scalar color = scalar;
        for (int i = 0; i < 4; i++) {
            Imgproc.line(finalImage, new Point((int) Q[i][0], (int) Q[i][1]),
                    new Point((int) Q[(i + 1) % 4][0], (int) Q[(i + 1) % 4][1]), color, 4, 4, 0);
     ...
     */
    public static boolean checkQuadrilateral(double [][] scene_corners, Rect roi) {
     ...
     */
    public static Rect bestBoundingBoxFast(ArrayList<Point> list, double accuracy) {
        if (accuracy < 0.0 || accuracy > 1.0) {
            System.err.println("Not valid accuracy [0.0, 1.0]");
            accuracy = .9;
        }
        int N = list.size(), n = (int)Math.ceil(N * accuracy);
        Rect best = new Rect(0, 0, 1 << 15, 1 << 15); // 'infinite' rectangle

        ArrayList<Point> xSorted = (ArrayList<Point>)list.clone();
        ArrayList<Point> ySorted = (ArrayList<Point>)list.clone();
				...
				*/
                ...
                for (int pup = N - 1, pright = i - 1; pup >= j ;  ) {
                    ...
                    if ((xMax - xMin) * (yMax - yMin) < best.area()) {
                        best = new Rect((int)xMin, (int)yMin, (int)(xMax - xMin), (int)(yMax - yMin));
                    }
